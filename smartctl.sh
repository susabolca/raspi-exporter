...
# smart
for i in $(cd /dev && ls sd?); do 
smartctl -d sat -f hex -A -i -P ignore /dev/$i | grep 0x | while read l; do 
v=($l)
echo ${pre}_smartctl { dev=\"$i\",id=\"${v[0]}\",value=\"${v[3]}\",worst=\"${v[4]}\",thresh=\"${v[5]}\",when_failed=\"${v[8]}\",name=\"${v[1]}\" } ${v[9]}
done # while read
done # for i 
